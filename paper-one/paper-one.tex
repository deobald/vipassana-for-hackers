...
% HACK: ignore \newfontfamily for LaTeXML (epub), otherwise
%       the optional paremeters are printed in the doc
\newfontfamily\fontspecisworking{NotoSansDevanagari-Regular.ttf}
\ifx\fontspecisworking\undefined
  \message{fontspec is not working; ignoring newfontfamily commands}
  \message{...this is probably because you are generating an epub.}
\else
  \newfontfamily\devanagarifontsans{NotoSansDevanagari-Regular.ttf}[ BoldFont = NotoSansDevanagari-Bold.ttf ]
  \newfontfamily\devanagarifontserif{NotoSerifDevanagari-Regular.ttf}[ BoldFont = NotoSerifDevanagari-Bold.ttf ]
\fi

% HACK: ignore \devanagarifontserif for LaTeXML (epub)
\ifx\devanagarifontserif\undefined
  \let\devanagarifontserif\textnormal
\fi
...
The equivalent Hindi/Sanskrit word, {\devanagarifontserif संस्कार} (sanskaar) roughly translates in this context to ``mental impression''. This second definition will apply to this paper.
